    ...
    <div className="flex h-full min-h-0 w-full flex-col gap-4">
      <div className="flex shrink-0 flex-wrap items-start justify-between gap-4">
        <div className="space-y-2">
          ...
          {(transfer.status === 'draft' || !isAcceptable) && (
            <p className="text-muted-foreground text-sm">
              {transfer.status === 'draft'
                ? 'Transfer hali jarayonda. Yuborilgach qabul qilishingiz mumkin bo\'ladi.'
                : 'Transfer qabul qilingan. Ma\'lumotlarni ko\'rishingiz mumkin.'}
            </p>
          )}
        </div>
      ...
      <div className={`${LIST_PAGE_TABLE_SECTION_CLASS} lg:grid lg:grid-cols-[1fr_auto] lg:gap-4`}>
        ...
        <div className="text-muted-foreground shrink-0 grid gap-2 px-1 text-sm sm:grid-cols-2">
          <div>
            <span className="text-muted-foreground">Yuboruvchi ombor:</span>{' '}
            <span className="text-foreground">{transfer.fromWarehouseName}</span>
          </div>
          <div>
            <span className="text-muted-foreground">Qabul qiluvchi ombor:</span>{' '}
            <span className="text-foreground">{transfer.toWarehouseName}</span>
          </div>
          <div>
