...
function OrderDraftEditor({ orderId, order }: OrderDraftEditorProps) {
  ...
  async function handleConfirmOrder() {
    ...
    try {
      ...
      await runOrderReceiptPrintFlow(confirmedOrder, recordReceipt)
...
export function OrderEditPage() {
  ...
  if (!isDraft) {
    return <OrderDetailView order={order} listPath={ORDERS_LIST_PATH} />
  }

  return <OrderDraftEditor key={order.id} orderId={id} order={order} />
}
